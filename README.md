...
8. 发布失败时，插件会在失败消息中附带可解析标签；用户可回复该失败消息并发送“继续 <补充指令>”自动 `resume` 同一 session 重试发布。
...
- `/发布文章 <文章ID> [空间] [团队] [知识库名称]`：发布已解析成功的文章到知识库；缺省按“空间→团队→知识库”顺序生效：传 3+ 参数按“空间/团队/知识库”解析，传 2 参数时仅在已设置默认空间时按“团队/知识库”解析，传 1 参数时仅在已设置默认空间与默认团队时按“知识库”解析，不支持跳级缺省。也支持回复发布引导消息自动触发；若发布失败可回复失败消息“继续 <补充指令>”基于同一 session 重试。
